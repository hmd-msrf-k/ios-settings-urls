...
- 显示与亮度 → 全天候显示: `bridge:root=DISPLAY_AND_BRIGHTNESS&path=TRITIUM_ID`
- 显示与亮度 → 全天候显示 → 全天候显示: `bridge:root=DISPLAY_AND_BRIGHTNESS&path=TRITIUM_ID#TRITIUM_ID`
- 显示与亮度 → 全天候显示 → 手腕垂下: `bridge:root=DISPLAY_AND_BRIGHTNESS&path=TRITIUM_ID#TRITIUM_PRIVACY_GROUP_ID`
- 显示与亮度 → 全天候显示 → 显示通知: `bridge:root=DISPLAY_AND_BRIGHTNESS&path=TRITIUM_ID/Show%20Notifications`
- 显示与亮度 → 全天候显示 → 显示通知 → 显示通知: `bridge:root=DISPLAY_AND_BRIGHTNESS&path=TRITIUM_ID/Show%20Notifications#APP_BACKLIGHT_PRIVACY_GLOBAL_APP`
- 显示与亮度 → 全天候显示 → 显示通知 → APPLE WATCH APP: `bridge:root=DISPLAY_AND_BRIGHTNESS&path=TRITIUM_ID/Show%20Notifications#APP_GROUP_ID`
- 显示与亮度 → 全天候显示 → 显示App: `bridge:root=DISPLAY_AND_BRIGHTNESS&path=TRITIUM_ID/Show%20Apps`
- 显示与亮度 → 全天候显示 → 显示App → 显示App: `bridge:root=DISPLAY_AND_BRIGHTNESS&path=TRITIUM_ID/Show%20Apps#APP_BACKLIGHT_PRIVACY_GLOBAL_APP`
- 显示与亮度 → 全天候显示 → 显示App → App: `bridge:root=DISPLAY_AND_BRIGHTNESS&path=TRITIUM_ID/Show%20Apps#APP_GROUP_ID`
...
- 通用 → 关于本机 → 有限保修 → 保障范围详细信息: `bridge:root=GENERAL_LINK&path=ABOUT/WARRANTY_DESCRIPTION#COVERAGE%20DETAILS`
- 通用 → 关于本机 → 有限保修 → 硬件保障范围: `bridge:root=GENERAL_LINK&path=ABOUT/WARRANTY_DESCRIPTION#Hardware%20Coverage`
...
- 通用 → 浸入水中时自动启动 → 自动启动音频App: `bridge:root=GENERAL_LINK&path=AUTO_LAUNCH#AUTO_LAUNCH_MEDIA`
...
- 通用 → 语言与地区 → 自定义: `bridge:root=GENERAL_LINK&path=LANGUAGE_AND_REGION#CUSTOM_ID`
...
- 声效与触感反馈 → 触感反馈: `bridge:root=SOUNDS_ID#Haptics`
- 声效与触感反馈 → 触感反馈提醒: `bridge:root=SOUNDS_ID#Haptic%20Alerts`
- 声效与触感反馈 → 表冠触感反馈: `bridge:root=SOUNDS_ID#JACKRABBIT_ID`
- 声效与触感反馈 → 系统触感反馈: `bridge:root=SOUNDS_ID#SYSTEM_HAPTICS_ID`
- 声效与触感反馈 → 覆屏静音: `bridge:root=SOUNDS_ID#COVER_TO_MUTE`
...
- 辅助功能 → 钟鸣: `bridge:root=ACCESSIBILITY&path=TapticChimesCell`
- 辅助功能 → 钟鸣 → 钟鸣: `bridge:root=ACCESSIBILITY&path=TapticChimesCell#Chimes`
- 辅助功能 → 钟鸣 → 定时: `bridge:root=ACCESSIBILITY&path=TapticChimesCell/Schedule`
- 辅助功能 → 钟鸣 → 定时 → 每小时: `bridge:root=ACCESSIBILITY&path=TapticChimesCell/Schedule#Hourly`
- 辅助功能 → 钟鸣 → 定时 → 30分钟: `bridge:root=ACCESSIBILITY&path=TapticChimesCell/Schedule#30%20minutes`
- 辅助功能 → 钟鸣 → 定时 → 15分钟: `bridge:root=ACCESSIBILITY&path=TapticChimesCell/Schedule#15%20minutes`
- 辅助功能 → 钟鸣 → 声音: `bridge:root=ACCESSIBILITY&path=TapticChimesCell/Sounds`
- 辅助功能 → 钟鸣 → 声音 → 铃铛: `bridge:root=ACCESSIBILITY&path=TapticChimesCell/Sounds#Bells`
- 辅助功能 → 钟鸣 → 声音 → 鸟鸣: `bridge:root=ACCESSIBILITY&path=TapticChimesCell/Sounds#Birds`
...
- 辅助功能 → 触控调节 → 按住时长: `bridge:root=ACCESSIBILITY&path=TouchAccommodationsCell#HoldDurationGroup`
- 辅助功能 → 触控调节 → 按住时长: `bridge:root=ACCESSIBILITY&path=TouchAccommodationsCell#HoldDuration`
- 辅助功能 → 触控调节 → 忽略重复触控: `bridge:root=ACCESSIBILITY&path=TouchAccommodationsCell#IgnoreRepeatGroup`
