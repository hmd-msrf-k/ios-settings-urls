...
- 画面表示と明るさ → 常にオン → 通知を表示 → APPLE WATCHのアプリ: `bridge:root=DISPLAY_AND_BRIGHTNESS&path=TRITIUM_ID/Show%20Notifications#APP_GROUP_ID`
- 画面表示と明るさ → 常にオン → アプリを表示: `bridge:root=DISPLAY_AND_BRIGHTNESS&path=TRITIUM_ID/Show%20Apps`
- 画面表示と明るさ → 常にオン → アプリを表示 → アプリを表示: `bridge:root=DISPLAY_AND_BRIGHTNESS&path=TRITIUM_ID/Show%20Apps#APP_BACKLIGHT_PRIVACY_GLOBAL_APP`
- 画面表示と明るさ → 常にオン → アプリを表示 → アプリ: `bridge:root=DISPLAY_AND_BRIGHTNESS&path=TRITIUM_ID/Show%20Apps#APP_GROUP_ID`
...
- 一般 → 水中にいるとき自動起動 → オーディオアプリを自動起動: `bridge:root=GENERAL_LINK&path=AUTO_LAUNCH#AUTO_LAUNCH_MEDIA`
...
- 一般 → 時計に戻る → アプリ: `bridge:root=GENERAL_LINK&path=RETURN_TO_CLOCK#APP_GROUP_ID`
...
- アクセシビリティ → チャイム: `bridge:root=ACCESSIBILITY&path=TapticChimesCell`
- アクセシビリティ → チャイム → チャイム: `bridge:root=ACCESSIBILITY&path=TapticChimesCell#Chimes`
- アクセシビリティ → チャイム → スケジュール: `bridge:root=ACCESSIBILITY&path=TapticChimesCell/Schedule`
- アクセシビリティ → チャイム → スケジュール → 正時: `bridge:root=ACCESSIBILITY&path=TapticChimesCell/Schedule#Hourly`
- アクセシビリティ → チャイム → スケジュール → 30分: `bridge:root=ACCESSIBILITY&path=TapticChimesCell/Schedule#30%20minutes`
- アクセシビリティ → チャイム → スケジュール → 15分: `bridge:root=ACCESSIBILITY&path=TapticChimesCell/Schedule#15%20minutes`
- アクセシビリティ → チャイム → サウンド: `bridge:root=ACCESSIBILITY&path=TapticChimesCell/Sounds`
- アクセシビリティ → チャイム → サウンド → ベル: `bridge:root=ACCESSIBILITY&path=TapticChimesCell/Sounds#Bells`
- アクセシビリティ → チャイム → サウンド → 鳥: `bridge:root=ACCESSIBILITY&path=TapticChimesCell/Sounds#Birds`
- アクセシビリティ → 動作: `bridge:root=ACCESSIBILITY#MotorGroupCell`
...
- アクセシビリティ → タッチ調整 → 保持継続時間: `bridge:root=ACCESSIBILITY&path=TouchAccommodationsCell#HoldDurationGroup`
- アクセシビリティ → タッチ調整 → 保持継続時間: `bridge:root=ACCESSIBILITY&path=TouchAccommodationsCell#HoldDuration`
- アクセシビリティ → タッチ調整 → 繰り返しを無視: `bridge:root=ACCESSIBILITY&path=TouchAccommodationsCell#IgnoreRepeatGroup`
